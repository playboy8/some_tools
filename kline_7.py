...
class KLineWidget(KeyWraper):
    ...
    def makePI(self, name):
        ...
        vb = CustomViewBox()
        # 创建一个新的AxisItem对象
        newAxis = pg.AxisItem(orientation='bottom')
        # 将其连接到当前plotItem对象上
        #plotItem.axes['bottom']['item'] = newAxis
        # 设置坐标轴标签和位置等属性
        #newAxis.setLabel(text='X Axis Label')
        #newAxis.setRange(xRange)
        
        plotItem = pg.PlotItem(viewBox=vb, name=name, axisItems={'bottom': newAxis})
    ...
    def loadData(self, datas, sigs=None):
        ...
        """
        # 设置中心点时间
        # 绑定数据，更新横座标映射，更新Y轴自适应函数，更新十字光标映射
...
class GetData():
    ...
    def getData_Tushare(self, code, ktype='5', start=str(dt.date.today() - dt.timedelta(days=1000)),
        ...
        try:
            pro = ts.pro_api('bc462a4181ec6cd18f6983adce52e99841f648450f2cc38c549529b1')            
            self.k_data = pro.daily(ts_code=code,start_date=start, end_date=end )
            print('====data:', self.k_data)
            self.k_data.rename(columns={'trade_date': 'datetime'}, inplace=True)
            self.k_data.rename(columns={'vol': 'volume'}, inplace=True)
            self.k_data.rename(columns={'amount': 'openInterest'}, inplace=True)
